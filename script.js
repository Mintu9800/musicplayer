...
body.addEventListener("click", (event) => {
  ...
  if (event.target.dataset.songOptionToggle !== undefined) {
    ...
    parent.querySelector(".song-card-options").classList.toggle("open-options");
  }
  ...
  if (event.target.classList.contains("albumCard")) {
    ...
    const url =
      ...
      "?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi";
...
function switchPage(elementLink) {
  ...
  [...pages].forEach((page) => {
    if (!page.classList.contains("hide-page")) page.classList.add("hide-page");
  });
...
async function insertArtistDetails(event) {
  ...
  if (event.target.dataset.index <= 2) {
    const target = document.querySelector('[data-index="2"].artist-container');
    target.insertAdjacentElement("afterend", artistDropDown);
  } else if (event.target.dataset.index <= 5) {
    const target = document.querySelector('[data-index="5"].artist-container');
    target.insertAdjacentElement("afterend", artistDropDown);
  } else if (event.target.dataset.index <= 8) {
    const target = document.querySelector('[data-index="8"].artist-container');
    target.insertAdjacentElement("afterend", artistDropDown);
  } else if (event.target.dataset.index <= 11) {
    const target = document.querySelector('[data-index="11"].artist-container');
    target.insertAdjacentElement("afterend", artistDropDown);
  } else if (event.target.dataset.index <= 14) {
    const target = document.querySelector('[data-index="14"].artist-container');
    target.insertAdjacentElement("afterend", artistDropDown);
  }
  ...
  try {
    ...
    const artistGenre = genres["genres"].map((genre) => genre.name).join(", ");
    loadingClone.remove();
    ...
    fetch(
      firstAlbum + "?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi"
    )
      .then((response) => response.json())
      .then((response) => {
        return response["tracks"];
...
fetch(
  "https://api.napster.com/v2.1/tracks/top?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi",
  requestOptions
)
  .then((response) => response.json())
  .then((result) => {
...
fetch(
  "https://api.napster.com/v2.2/artists/top?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi&range=life&limit=17",
  requestOptions
)
  .then((response) => response.json())
  .then((result) => {
    ...
    requiredArtist.forEach((artist, index) => {
      ...
      const element = `
        ...
        artist.genre
      }?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi" data-album-link="${
        artist.albums
      }?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi" data-artist-id="${
        artist.id
...
fetch(
  "https://api.napster.com/v2.2/playlists/featured?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi&limit=3"
)
  .then((response) => response.json())
    ...
    playlists.forEach((playlist) => {
      ...
      fetch(
        playlistItem.dataset.tracksUrl +
          "?apikey=NjU5ZDZlZjEtMDRiOC00NjI1LWJiN2ItNDczZDQ1NDA4YjFh"
      )
        .then((response) => response.json())
...
document.querySelector("form").onsubmit = async (e) => {
  e.preventDefault();
  const searchQuery = e.target.querySelector('input[type="text"]').value.trim();
...
async function searchSong(query, filter) {
  try {
    let response = await fetch(
      `https://api.napster.com/v2.2/search?apikey=OWE4ZWNkYjAtY2MxOC00NzNjLWE0ZDItNDU1NGMzYTllOGVi&query=${query}&type=track&per_type_limit=16`
    );
...
audio.addEventListener("loadedmetadata", (event) => {
  console.log("started");
  document.querySelector("#audio-play").classList.remove("hide-audio-control");
...
function renderQueue() {
  if (currentlyPlaying.length > 0) {
    ...
    const element = `
        ...
        `;
    document.querySelector(".currently-playing-container").innerHTML = element;
  }
  if (queue.length === 0) {
...
function activateAudio(song) {
  ...
  document.querySelector(".music-info .track-text").textContent = song.songName;
